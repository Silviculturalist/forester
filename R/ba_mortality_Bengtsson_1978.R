...
#'
#'
#' Bengtsson_1978_BA_mortality
#' ba_mortality_thinned_Bengtsson_1978 <- function(mainSpecies,standAge,DBH,county,NormProduktion,floor=0.01,ciel=0.09){
#'    #Recommendation due to grouped regression, according to Heureka manual.
#'   if(standAge<30){
#'     standAge <- 30
#'   } else if(standAge>120){
#'     standAge <- 120
#'   }
#'
#'   #Recommendation due to grouped regression, according to Heureka manual.
...
#'   #Northern Sweden here "Norrland och Dalarna", p. 270.
#'   #Counties interpreted to be included see list below.
#'   northern_sweden <- ifelse(county %in%  c("Norrbottens lappmark",
#'                                                               "Norrbottens kustland",
#'                                                               "Västerbottens lappmark",
#'                                                               "Västerbottens kustland",
#'                                                               "Västernorrland - Ångermanlands landskap",
...
#'
#'   if (northern_sweden==1){
#'     if(mainSpecies == "Pinus sylvestris"){
#'       a <- 0.00437
#'       b1 <- -0.000847
#'       b2 <- 0.0000214
#'       b3 <- 0.000104
...
#'       NormProduktion <- 0
#'
#'     } else if(mainSpecies == "Picea abies"){
#'       a <- 0.00390
#'       b <- -0.000610
#'       b2 <- 0.0000160
#'       b3 <- 0.000050
...
#'       NormProduktion <- 0
#'
#'     } else if(forester::tree_type(mainSpecies)=="Deciduous"){
#'       a <- 0.01005
#'       b1 <- -0.000843
#'       b2 <- 0.0000176
#'       b3 <- -0.00000174
...
#'   if(northern_sweden==1){
#'     return(
#'       (a+(b1*DBH) + (b2*(DBH^2)) + (b3*standAge) + (b4*(standAge^2)) + (b5*NormProduktion))*0.6
#'     )
#'   } else {
#'     return(
#'       (a+(b1*DBH) + (b2*(DBH^2)) + (b3*standAge) + (b4*(standAge^2)) + (b5*NormProduktion))*0.75
#'     )
#'   }
#'
#' }
