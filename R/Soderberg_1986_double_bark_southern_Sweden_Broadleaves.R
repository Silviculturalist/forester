#' Double bark thickness in southern Sweden for Broadleaves from Söderberg 1986.
#'
...
#' *Gotland
#'
#' @details Multiple correlation coefficient R = 0.82
#'
...
Soderberg_1986_double_bark_southern_Sweden_Broadleaves <- function(
  ...
  return(
    exp(
      +0.99684*10^(-2)*(diameter_cm_under_bark*10)+
        -0.13735*10^(-4)*((diameter_cm_under_bark*10)^2)+
        +0.75334*10^(-2)*age_at_breast_height+
        -0.32951*10^(-4)*(age_at_breast_height^2)+
        +1.8722*latitude+
