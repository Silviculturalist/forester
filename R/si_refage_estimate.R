#' Site Index Estimations for Sweden
...
#' @param refAge Forecasted age. OBS, refAge cannot be set for Alnus spp., Aspen or Betula (==50)
#' @param ...
#'
...
si_refage_estimate <- function(species=c("NorwaySpruce","ScotsPine","Larch","Beech","Oak","Birch","AlnusIncana","AlnusGlutinosa","Aspen"),
                           age, height, refAge=100){
  ...
  if(species!="NorwaySpruce"){
  r <- (((height-d)^2)+(4*parambeta*height*(age^paramb2)))^0.5
  } else if (species == "NorwaySpruce"){
  r <- (((height-d)^2)+(4*parambeta*height*((age-3)^paramb2)))^0.5
  }

  (height+d+r) / (2+(4*parambeta*(refAge^paramb2)) / (height-d+r))
